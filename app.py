...
# Dritter Reiter: dedizierte Good-News-Quelle. Die fünf Stamm-Outlets oben
# haben keine eigene "Gute Nachrichten"-Rubrik, daher wird hier stattdessen
# goodnews.eu (Good Impact / Noah Foundation) genutzt, das werktäglich
# positive, lösungsorientierte Meldungen aus deutschsprachigen Medien sammelt.
# Falls dieser Feed sich mal ändert oder umzieht: URL hier einfach anpassen,
# der Rest der App bleibt unberührt.
POSITIVE_NEWS_FEEDS = (
...
# WordPress/Yoast hängt an automatisch erzeugte Beschreibungen oft einen
# Boilerplate-Satz wie "The post ... appeared first on ..." bzw. dessen
# deutsches Pendant an. Der wird beim Säubern der Beschreibung entfernt.
WP_BOILERPLATE_PATTERNS = (
    re.compile(r"\s*The post .+ appeared first on .+\.\s*$", re.IGNORECASE),
    re.compile(r"\s*Der Beitrag .+ erschien zuerst auf .+\.\s*$", re.IGNORECASE),
)
...
def clean_description(raw_html):
    ...
    text = re.sub(r"\s*mehr\.\.\.$", "", text)
    for pattern in WP_BOILERPLATE_PATTERNS:
...
def _dedupe_top(articles, limit=ARTICLES_PER_OUTLET):
    seen_links = set()
    result = []
    for a in articles:
...
def fetch_rss(url, limit=ARTICLES_PER_OUTLET):
    try:
        articles = _parse_feed(url)
    except Exception:
        return []
    return _dedupe_top(articles, limit=limit)


@st.cache_data(ttl=3600)
def fetch_rss_combined(urls, limit=ARTICLES_PER_OUTLET):
    combined = []
    for url in urls:
        try:
    ...
    return _dedupe_top(combined, limit=limit)
...
.ff-app-shell:has(#ff-tab-positiv:checked) #ff-panel-positiv {
    display: block;
}
.ff-rule {
    border: none;
...
def render_positive_news_html(articles):
    if articles:
        rows = []
        for index, a in enumerate(articles):
            lead_class = " ff-positive-headline" if index == 0 else ""
            content = (
                f'<a href="{a["link"]}" target="_blank" '
                f'class="ff-headline{lead_class}">{a["title"]}</a>'
            )
            if a.get("description"):
                content += f'<span class="ff-description">{a["description"]}</span>'
            rows.append(f'<div class="ff-article">{content}</div>')
    ...
    return (
        '<div class="ff-positive-wrap"><div class="ff-positive-card">'
        '<div class="ff-positive-accent"></div>'
        '<div class="ff-positive-card-body">'
        '<div class="ff-positive-tag">good news &middot; goodnews.eu</div>'
        f"{body}"
        "</div></div></div>"
    )
...
def main():
    ...
    positive_html = render_positive_news_html(
        fetch_rss_combined(POSITIVE_NEWS_FEEDS, limit=ARTICLES_POSITIVE)
    )
    ...
    st.markdown(
        ...
        "Außenpolitik</label>"
        '<label class="ff-toggle-btn">'
        '<input type="radio" name="ff-tab" id="ff-tab-positiv" class="ff-tab-input">'
        "Positive Nachrichten</label>"
        "</div>"
        "</div>"
        '<hr class="ff-rule">'
        f'<div id="ff-panel-innen" class="ff-panel">{innen_html}</div>'
        f'<div id="ff-panel-aussen" class="ff-panel">{aussen_html}</div>'
        f'<div id="ff-panel-positiv" class="ff-panel">{positive_html}</div>'
        "</div>",
        unsafe_allow_html=True,
    )

    st.markdown(
        '<div class="ff-footer">Quellen: taz · Die Zeit · Tagesschau · FAZ · NZZ · '
        'Positive Nachrichten: goodnews.eu</div>',
        unsafe_allow_html=True,
    )
